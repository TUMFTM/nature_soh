...
if __name__ == "__main__":
    ...
    vca = VCABase()
    diff_v = vca.differentiate_signal(time=df["time_h"],signal=df["hv_battery_voltage"])
    cap = vca.calculate_capacity_signal(time=df["time_h"],current=df["hv_battery_current"],initial=0)
    cropped_mask = vca.crop_signal_mask(signal=df["hv_battery_voltage"],limit=(400,600))
    normalized_v = vca.normalize_signal(signal=df["hv_battery_voltage"],factor=np.max(df["hv_battery_voltage"]))
