...
if __name__ == "__main__":
    ...
    def filter_func(signal):
        filtered_signal = FilterMethods().rolling_mean_numpy(signal,
                                                       window_size=FilterMethods().round_to_next_odd_number(
                                                           1 / 100 * len(signal)))
        return filtered_signal


    ica_extractor.set_filter(filter_func)
    df["capacity"] = ica_extractor.calculate_capacity_signal(df["time_h"],df["hv_battery_current"],initial=0)
    ica,voltage = ica_extractor.get_normalized_ica(time=df["time_h"],
