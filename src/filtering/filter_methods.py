...
class FilterMethods():
    ...
    def butter_lowpass(self,signal,cutoff_freq , fs, order, butter_dict = FilteringConfig.butter_dict.value):
        ...
        b, a = butter(N=order,
                      ...
                      btype=butter_dict["btype"],
                      analog=butter_dict["analog"])
        return filtfilt(b, a, signal) # use filter forward and backward
...
if __name__ == "__main__":
    ...
    savgol = FilterMethods().savgol(signal = df["hv_battery_current"],
                                                 window_size = 0.04*len(df["hv_battery_current"]),
                                                 savgol_dict=FilteringConfig.savgol_dict.value)

    mean =  FilterMethods().rolling_mean(signal = df["hv_battery_current"],
                                       window_size =  0.04*len(df["hv_battery_current"]),
                                       mean_dict=FilteringConfig.mean_dict.value)

    butter =  FilterMethods().butter_lowpass(signal = df["hv_battery_current"],
                                           cutoff_freq = 400,
                                           fs = 1000,
                                           order = 5,
                                           butter_dict = FilteringConfig.butter_dict.value)
